...
Plugin Name: Affinitomics + Taxonomy Converter
Description: Easily Import your categories and tags into Affintomics!
Author: Hans Hauge
Author URI: https://github.com/HansHauge
Version: 0.9
Forked From: Taxonomy Converter by kristarella
License: GPLv2
*/
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class WP_Affinitomics_Taxonomy_Converter extends WP_Importer {
	...
	function header($current='category') {
		...
		if ( ! current_user_can('manage_categories') ) {
			...
			echo '<p>' . __('Cheatin&#8217; uh?', 'wptaxconvertaffinitomics') . '</p>';
			echo '</div>';
		} else { ?>
			...
			<?php foreach ($this->taxes as $name => $tax) {
				if ($name == $current)
					...
					$classes = 'nav-tab';
			?>
			<a class="<?php echo $classes; ?>" href="admin.php?import=wptaxconvertaffinitomics&amp;tax=<?php echo $name; ?>"><?php echo $tax->label; ?></a>
			<?php } ?>
			</h2>
		<?php }
	}
	...
	function tabs($tax) {
		...
		if ( $num > 0 ) {
			screen_icon();
			echo '<h2>' . __( 'Convert or Copy '.$details->label, 'wptaxconvertaffinitomics') . '</h2>';
			echo '<div class="narrow">';
			if ($details->label == 'Categories'){
				...
				echo '<p>' . __('
					...
					select the terms (below), then click the Go button.', 'wptaxconvert') . '</p>';
			}
			echo '<p>' . __('
					Keep in mind that "converted" terms are deleted from original taxonomy, and if you convert a term with children,
					the children become top-level orphans.', 'wptaxconvert') . '</p></div>';

			$this->terms_form($tax);
		} else {
			echo '<p>'.__('You have no terms to convert', 'wptaxconvertaffinitomics').'</p>';
		}
	}
...
function check_all_rows() {
	...
	if ( 'false' == checkflag ) {
		...
		checkflag = 'true';
		return '<?php _e('Uncheck All', 'wptaxconvertaffinitomics') ?>';
	} else {
		for ( i = 0; i < field.length; i++ ) {
			if ( 'terms_to_convert[]' == field[i].name )
		...
		checkflag = 'false';
		return '<?php _e('Check All', 'wptaxconvertaffinitomics') ?>';
	}
}
/* ]]> */
</script>

<form name="term_list" id="term_list" action="admin.php?import=wptaxconvertaffinitomics&amp;tax=<?php echo $tax; ?>&amp;step=2" method="post">
...
<?php
	foreach ($this->taxes as $name => $details) {
		if ( ($details->label == 'Categories') && ($name == $tax) ){
			echo '<label><input type="checkbox" name="taxes[]" value="descriptor" checked="checked"> Descriptors &emsp;</label>';
			continue;
		} else if ( ($details->label == 'Tags') && ($name == $tax) ) {
			echo '<label><input type="checkbox" name="taxes[]" value="draw" checked="checked"> Draws &emsp;</label>';
			continue;
		} else if ($name == $tax) {
			continue;
		} else {
			echo '<label><input type="checkbox" name="taxes[]" value="' . $name . '"> ' . $details->label . '&emsp;</label>';
		}
	}
?>
</p>

<p><input type="button" class="button-secondary" value="<?php esc_attr_e('Check All', 'wptaxconvertaffinitomics'); ?>" onclick="this.value=check_all_rows()" />
...
<?php
	if ( ! empty($this->hybrids_ids) )
			echo '<p><a name="note"></a>' . __('* This term is already in another taxonomy, converting will add the new taxonomy term to existing posts in that taxonomy.', 'wptaxconvertaffinitomics') . '</p>'; ?>

<p class="submit"><input type="submit" name="submit" class="button button-primary" value="<?php esc_attr_e('Go!', 'wptaxconvertaffinitomics'); ?>" /></p>
</form>
...
<?php	} ?>
	...
	function process($tax) {
		...
		if ( (!isset($_POST['terms_to_convert']) || !is_array($_POST['terms_to_convert'])) && empty($this->terms_to_convert) || (!isset($_POST['taxes'])) ) { ?>
			<div class="narrow">
			<p><?php printf(__('Uh, oh. Something didn&#8217;t work. Please <a href="%s">try again</a>.', 'wptaxconvertaffinitomics'), 'admin.php?import=wptaxconvertaffinitomics&amp;tax='.$tax); ?></p>
			</div>
<?php		return;
		}
		...
		foreach ( (array) $this->terms_to_convert as $term_id) {
			...
			if ( empty($exists) ) {
				echo '<li>' . sprintf( __('Term %s doesn&#8217;t exist in '.$taxonomy->label.'!', 'wptaxconvertaffinitomics'),  $term_id ) . "</li>\n";
			} else {
				// if the term exist do the copy/convert
				// $term is the existing term
				$term = get_term($term_id,$tax);
				echo '<li>' . sprintf(__($c_label.'ing term <strong>%s</strong> ... ', 'wptaxconvertaffinitomics'),  $term->name);
				...
				foreach ($new_taxes as $new_tax) {
					...
					echo __($c_label.' successful.', 'wptaxconvertaffinitomics') . "</li>\n";
				}
			}
		}
		echo '</ul>';
		...
		if ( $hybrid_cats )
			echo '<p>' . __('* This term is now in multiple taxonomies. The converter has added the new term to all posts with the original taxonomy term.', 'wptaxconvertaffinitomics') . '</p>';
		echo '<p>' . sprintf( __('We&#8217;re all done here, but you can always <a href="%s">convert more</a>.', 'wptaxconvertaffinitomics'), 'admin.php?import=wptaxconvertaffinitomics' ) . '</p>';
	}
